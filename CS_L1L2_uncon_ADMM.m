...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%         min_x 0.5||Ax-b||^2 + lambda(|x|_1- alpha |x|_2)             %%%  
%%%                                                                     %%%
...
AAt     = A*A';
L       = chol(speye(M) + 1/delta*AAt, 'lower'); %speye:稀疏单位矩阵 chol:分解成上三角矩阵,lower：选用下三角形(是对称的)
L       = sparse(L); %稀疏格式存储
U       = sparse(L');   
...
for it = 1:maxit                
    %update x
    x       = shrinkL12(y - u, lambda/delta, alpha); %自定义函数1
    if it == 1
        x   = x0;
    end
